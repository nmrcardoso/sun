...
__global__ void  kernel_calc_Fg_theta_delta(complex *array, complex *res_save, complex *Delta){
	...
	if(idd < DEVPARAMS::Volume){
		...
		//Downlinks
		for(int nu = 0; nu < DIR; nu++) 	
			delta += GAUGE_LOAD<UseTex, atype, Real>( array, neighborEOIndexMinusOne(id, oddbit, nu) + nu * DEVPARAMS::Volume, offset);
		delta = (delta-delta.dagger()).subtraceunit();
        //Save Delta
        DELTA_SAVE<atypedelta, Real>( Delta, delta , fullLatticeIndex(id, oddbit), DEVPARAMS::Volume);
		//theta
...
template <bool UseTex, ArrayType atype, ArrayType atypedelta, ArrayType atypelambda, class Real, int DIR>
__global__ void  kernel_calc_Fg_theta_delta(complex *array, complex *res_save, complex *Delta, complex *ILambda){
	int idd = INDEX1D();
	if(idd < DEVPARAMS::Volume){
		int id = idd;
		int oddbit = 0;
		...
		int offset = DEVPARAMS::Volume * 4;
		int idx = EOIndeX(id, oddbit);
		msun delta = LAMBDA_LOAD<UseTex, atypelambda, Real>( ILambda, idx, DEVPARAMS::Volume);
		//Uplinks
		for(int nu = 0; nu < DIR; nu++) 
			delta -= GAUGE_LOAD<UseTex, atype, Real>( array,  idx + nu * DEVPARAMS::Volume, offset);
		complex res;
...
private:
   ...
   void apply(const cudaStream_t &stream){
      TuneParam tp = tuneLaunch(*this, getTuning(), getVerbosity());
      kernel_pointer<<<tp.grid,tp.block, 0, stream>>>(array.GetPtr(), sum, delta.GetPtr(), ILambda.GetPtr());
	}

public:
   GaugeFixFFTLambdaQuality(gauge &array, gauge &delta, gauge &ILambda, int gridin[4]):array(array), delta(delta), ILambda(ILambda){
		functionName = "GaugeFixFFTLambdaQuality";
		value = complex::zero();
		kernel_pointer = NULL;
		if(array.EvenOdd()){	
			kernel_pointer = &kernel_calc_Fg_theta_delta<UseTex, atype, atypedelta, atypelambda, Real, DIR>;
		}
		if(kernel_pointer == NULL) errorCULQCD("No kernel GaugeFixFFTLambdaQuality function exist for this gauge array...");
		size = 1;
		for(int i=0;i<4;i++){
			grid[i]=gridin[i];
			size *= gridin[i];
   ...
   GaugeFixFFTLambdaQuality(gauge &array, gauge &delta, gauge &ILambda, complex *sum, \
   	int gridin[4]):array(array), delta(delta), ILambda(ILambda), sum(sum){
		functionName = "GaugeFixFFTLambdaQuality";
		value = complex::zero();
		kernel_pointer = NULL;
		if(array.EvenOdd()){	
   ...
   ~GaugeFixFFTLambdaQuality(){ dev_free(sum);};
	...
	void stat(){
		COUT << "GaugeFixFFTLambdaQuality:  " <<  time() << " s\t"  << bandwidth() << " GB/s\t" << flops() << " GFlops"  << endl;
	}
	...
	long long flop() const { 
		//not acccounting reduction!!!!!
		long long arrayflops = 2LL * DIR * array.getNumFlop(true) + delta.getNumFlop(false) + ILambda.getNumFlop(true);
		return (arrayflops + 2LL * NCOLORS * NCOLORS * (DIR + 1) + 4LL * NCOLORS * ( 1 + NCOLORS) ) * size;
	}

	long long bytes() const { 
		//not acccounting reduction!!!!!
		return (2LL * DIR * array.getNumParams() + delta.getNumParams() + ILambda.getNumParams() + 2LL) * size * sizeof(Real);
	}
...
public:
	...
	if(gx.EvenOdd() == false) errorCULQCD("gx not set in even/odd format...");
	if(delta.EvenOdd() == true) errorCULQCD("delta cannot be in even/odd format...");
	kernel_pointer = NULL;
...
__global__ void kernel_gauge_fix_U( complex *array, complex *gx){
	...
	if(idd < DEVPARAMS::Volume){
		...
		for(int nu = 0; nu < 4; nu++){
			...
			msun U_temp = g * U;
			//msun g0 = (GX_LOAD_DAGGER<UseTex, atypegx, Real>(gx,  neighborEOIndex(id, oddbit, nu, 1)));
			msun g0 = (GX_LOAD_DAGGER<UseTex, atypegx, Real>(gx,  neighborEOIndexPlusOne(id, oddbit, nu), DEVPARAMS::Volume));
			U = U_temp * g0;
...
public:
	...
	if(pgauge.EvenOdd() == false) errorCULQCD("gauge not set in even/odd format...");
	if(GX.EvenOdd() == false) errorCULQCD("gx not set in even/odd format...");
	kernel_pointer = NULL;
...
__forceinline__  __host__ __device__ int neighborNormalLatIndexPlusOne(int id, int parity, int mu){
	int x[4];
	getEOCoords3(x, id, DEVPARAMS::Grid, parity);
	x[mu] = (x[mu] + 1) % param_GridG(mu);
	int idx = ((((x[3] * param_GridG(2) + x[2]) * param_GridG(1)) + x[1] ) * param_GridG(0) + x[0]) ;
	return idx;
}
...
__forceinline__  __host__ __device__ int neighborNormalLatIndex(int id, int parity){
	int x[4];
	getEOCoords3(x, id, DEVPARAMS::Grid, parity);
	int idx = ((((x[3] * param_GridG(2) + x[2]) * param_GridG(1)) + x[1] ) * param_GridG(0) + x[0]) ;
	return idx;
...
__global__ void kernel_gauge_fix_U_NEW( complex *array, complex *delta, Real half_alpha){
	...
	msun de = DELTA_LOAD<UseTex, atypedelta, Real>( delta, neighborNormalLatIndex(id, parity));//, arg.threads);		
	msun g = msun::unit();
	g += de * half_alpha;
	reunit_link<Real>( &g );
	for(int nu = 0; nu < 4; nu++){
		msun U = GAUGE_LOAD<UseTex, atype, Real>( array, idd + nu * DEVPARAMS::Volume, DEVPARAMS::Volume * 4);	
		msun U_temp = g * U;
		de = DELTA_LOAD<UseTex, atypedelta, Real>(delta,  neighborNormalLatIndexPlusOne(id, parity, nu));	
		msun g0 = msun::unit();
		g0 += de * half_alpha;
		reunit_link<Real>( &g0 );
		U = U_temp * g0.dagger();
...
public:
	...
	half_alpha = alpha * 0.5;
	if(pgauge.EvenOdd() == false) errorCULQCD("gauge not set in even/odd format...");
	if(delta.EvenOdd() == true) errorCULQCD("delta are set in even/odd format...");
	kernel_pointer = NULL;
...
__global__ void fft_rotate_kernel_2D2D(GaugeFixFFTRotateArg<Real> arg){
	...
	if(direction==2){ 
		...
		int x0 = id % arg.X[0];
		int id  =  x0 + (x1 + (x2 + x3 * arg.X[2]) * arg.X[1]) * arg.X[0]; 
		int id_out =  (id>>1) + ((x0+x1+x2+x3)&1)*(arg.X[0] * arg.X[1] * arg.X[2] * arg.X[3]>>1);    
		arg.out[id_out] = arg.in[id];  
	}
}
template<typename Real>
...
private:
	...
	void apply(const cudaStream_t &stream){
		TuneParam tp = tuneLaunch(*this, getTuning(), getVerbosity());
		if(direction == 0)
			fft_rotate_kernel_2D2D<0, Real><<<tp.grid, tp.block, 0, stream>>>(arg);
		else if(direction == 1) 
			fft_rotate_kernel_2D2D<1, Real><<<tp.grid, tp.block, 0, stream>>>(arg);
		else if(direction == 2) 
			fft_rotate_kernel_2D2D<2, Real><<<tp.grid, tp.block, 0, stream>>>(arg);
		else 
			errorCULQCD("Error in GaugeFixFFTRotate option.\n");
	}
public:
...
: arg(arg) {
	...
	~GaugeFixFFTRotate () {}
	void Run(unsigned int dir, complex *data_in, complex *data_out, const cudaStream_t &stream){
		direction = dir;
		arg.in = data_in;
		arg.out = data_out;
		apply(stream);
...

template <bool UseTex, ArrayType atype, ArrayType atypeILambda, ArrayType atypeDeltax, ArrayType atypeGx, class Real, int DIR> 
complex CALL_FFT(gauge _pgauge, gauge ILambda, Real alpha, bool landautune, Real stopvalue, int maxsteps, int verbose, bool useGx){
	...
	printfCULQCD("Using 2D FFTs + 2D FFTs with cufftPlanMany using %d/%d elements...\n", _delta.getNumElems(), NCOLORS * NCOLORS);
	if(useGx){
		long long sizeused = _pgauge.Bytes() +  ILambda.Bytes() + _delta.Bytes() + _gx.Bytes() + PARAMS::Volume*sizeof(Real);
		COUT << "Device memory used not accounting CUFFT: " << sizeused/(1024*1024) << " MB" << endl;
	}
	else{
		long long sizeused =  _pgauge.Bytes() + ILambda.Bytes() + _delta.Bytes() + PARAMS::Volume*(sizeof(complex)+sizeof(Real));
		COUT << "Device memory used not accounting CUFFT: " << sizeused/(1024*1024) << " MB" << endl;
	}
	...
	//------------------------------------------------------------------------
	GaugeFixINVPSP<Real> invpsp(arg);
	GaugeFixFFTLambdaQuality<UseTex, atype, atypeDeltax, atypeILambda, Real, DIR> quality(\
		_pgauge, _delta, ILambda, _GxPtr, PARAMS::Grid);
	//if useGx=true
	GaugeFix_GX<UseTex, atypeDeltax, atypeGx, Real> calcGX(arg, alpha, _gx, _delta);
	GaugeFix<UseTex, atype, atypeGx, Real> gfix(_pgauge, _gx, arg);
	//if useGx=false
...
template <ArrayType atype, ArrayType atypeILambda, ArrayType atypeDeltax, ArrayType atypeGx, class Real, int DIR> 
complex CALL_FFT_TEX(gauge _pgauge, gauge ILambda, Real alpha, bool landautune, Real stopvalue, int maxsteps, int verbose, bool useGx){
	if(PARAMS::UseTex)
		return CALL_FFT<true, atype, atypeILambda, atypeDeltax, atypeGx, Real, DIR>(_pgauge, ILambda, alpha, landautune, stopvalue, maxsteps, verbose, useGx);
	else
		return CALL_FFT<false, atype, atypeILambda, atypeDeltax, atypeGx, Real, DIR>(_pgauge, ILambda, alpha, landautune, stopvalue, maxsteps, verbose, useGx);
}
template <ArrayType atype, ArrayType atypeILambda, ArrayType atypeDeltax, ArrayType atypeGx, class Real> 
complex CALL_FFT_DIR(gauge _pgauge, gauge ILambda, int DIR, Real alpha, bool landautune, Real stopvalue, int maxsteps, int verbose, bool useGx){
	if(DIR==3)
		return CALL_FFT_TEX<atype, atypeILambda, atypeDeltax, atypeGx, Real, 3>(_pgauge, ILambda, alpha, landautune, stopvalue, maxsteps, verbose, useGx);
	else
		return CALL_FFT_TEX<atype, atypeILambda, atypeDeltax, atypeGx, Real, 4>(_pgauge, ILambda, alpha, landautune, stopvalue, maxsteps, verbose, useGx);
}
template <ArrayType atype, ArrayType atypeILambda, ArrayType atypeDeltax, class Real> 
complex CALL_FFT_GX(gauge _pgauge, gauge ILambda, int DIR, Real alpha, bool landautune, Real stopvalue, int maxsteps, int verbose, bool useGx, ArrayType atypeGx){
	#if (NCOLORS == 3)
    if(atypeGx == SOA) return CALL_FFT_DIR<atype, atypeILambda, atypeDeltax, SOA, Real>(_pgauge, ILambda, DIR, alpha, landautune, stopvalue, maxsteps, verbose, useGx);
    else if(atypeGx == SOA12) return CALL_FFT_DIR<atype, atypeILambda, atypeDeltax, SOA12, Real>(_pgauge, ILambda, DIR, alpha, landautune, stopvalue, maxsteps, verbose, useGx);
    else if(atypeGx == SOA8) return CALL_FFT_DIR<atype, atypeILambda, atypeDeltax, SOA8, Real>(_pgauge, ILambda, DIR, alpha, landautune, stopvalue, maxsteps, verbose, useGx);
    else errorCULQCD("Not defined...\n");
    return complex::make_complex(9999.,999999999999.);
	#else
    return CALL_FFT_DIR<atype, atypeILambda, atypeDeltax, SOA, Real>(_pgauge, ILambda, DIR, alpha, landautune, stopvalue, maxsteps, verbose, useGx);	
    #endif
}
template <ArrayType atype, ArrayType atypeILambda, class Real> 
complex CALL_FFT_DELTA(gauge _pgauge, gauge ILambda, int DIR, Real alpha, bool landautune, Real stopvalue, int maxsteps, int verbose, ArrayType atypeDeltax, bool useGx, ArrayType atypeGx){
	#if (NCOLORS == 3)
    if(atypeDeltax == SOA) return CALL_FFT_GX<atype, atypeILambda, SOA, Real>(_pgauge, ILambda, DIR, alpha, landautune, stopvalue, maxsteps, verbose, useGx, atypeGx);
    else if(atypeDeltax == SOA12A) return CALL_FFT_GX<atype, atypeILambda, SOA12A, Real>(_pgauge, ILambda, DIR, alpha, landautune, stopvalue, maxsteps, verbose, useGx, atypeGx);
    else errorCULQCD("Not defined for SOA12 and SOA8...\n");
    return complex::make_complex(9999.,999999999999.);
	#else
    return CALL_FFT_GX<atype, atypeILambda, SOA, Real>(_pgauge, ILambda, DIR, alpha, landautune, stopvalue, maxsteps, verbose, useGx, atypeGx);	
    #endif
}
template <ArrayType atype, class Real> 
complex CALL_FFT_ILAMBDA(gauge _pgauge, gauge ILambda, int DIR, Real alpha, bool landautune, Real stopvalue, int maxsteps, int verbose, ArrayType atypeDeltax, bool useGx, ArrayType atypeGx){
	#if (NCOLORS == 3)
    if(ILambda.Type() == SOA && ILambda.EvenOdd()) return CALL_FFT_DELTA<atype, SOA, Real>(_pgauge, ILambda, DIR, alpha, landautune, stopvalue, maxsteps, verbose, atypeDeltax, useGx, atypeGx);
    else if(ILambda.Type() == SOA12A && ILambda.EvenOdd()) return CALL_FFT_DELTA<atype, SOA12A, Real>(_pgauge, ILambda, DIR, alpha, landautune, stopvalue, maxsteps, verbose, atypeDeltax, useGx, atypeGx);
    else errorCULQCD("Not defined for SOA12 and SOA8...\n");
    return complex::make_complex(9999.,999999999999.);
	#else
    return CALL_FFT_DELTA<atype, SOA, Real>(_pgauge, ILambda, DIR, alpha, landautune, stopvalue, maxsteps, verbose, atypeDeltax, useGx, atypeGx);	
    #endif
}
template <class Real> 
complex CALL_FFT_PGAUGE(gauge _pgauge, gauge ILambda, int DIR, Real alpha, bool landautune, Real stopvalue, int maxsteps, int verbose, ArrayType atypeDeltax, bool useGx, ArrayType atypeGx){
	#if (NCOLORS == 3)
    if(_pgauge.Type() == SOA) return CALL_FFT_ILAMBDA<SOA, Real>(_pgauge, ILambda, DIR, alpha, landautune, stopvalue, maxsteps, verbose, atypeDeltax, useGx, atypeGx);
    else if(_pgauge.Type() == SOA12) return CALL_FFT_ILAMBDA<SOA12, Real>(_pgauge, ILambda, DIR, alpha, landautune, stopvalue, maxsteps, verbose, atypeDeltax, useGx, atypeGx);
    else if(_pgauge.Type() == SOA8) return CALL_FFT_ILAMBDA<SOA8, Real>(_pgauge, ILambda, DIR, alpha, landautune, stopvalue, maxsteps, verbose, atypeDeltax, useGx, atypeGx);
    else errorCULQCD("Not defined...\n");
    return complex::make_complex(9999.,999999999999.);
	#else
    return CALL_FFT_ILAMBDA<SOA, Real>(_pgauge, ILambda, DIR, alpha, landautune, stopvalue, maxsteps, verbose, atypeDeltax, useGx, atypeGx);	
    #endif
}
...
*/ 
template <class Real> 
complex COVGaugeFixingFFT(gauge _pgauge, gauge ILambda, int DIR, Real alpha, bool landautune, Real stopvalue, int maxsteps, int verbose, ArrayType atypeDeltax, bool useGx, ArrayType atypeGx){ 
	#ifdef MULTI_GPU
	if(numnodes() > 1 ){
		COUT << "NOT IMPLEMENTED YET FOR MULTI GPUs!!!!!!!!!!!!!!!!!!!!!!!!!" << endl;
		return complex::make_complex(9999.,999999999999.);
	...
	COUT << "###############################################################################" << endl;
	string my_name ="";
	if(DIR==4) my_name = "Covariant Landau gauge fixing";
	else if(DIR==3) my_name =  "Covariant Coulomb gauge fixing";
	else{
        COUT << "DIR can only be 3, for Coulomb, or 4, for Landau.\nNo gauge fixing applied." << endl;
        return complex::make_complex(9999.,999999999999.);
	}
	...
	if(verbose < 1) verbose = maxsteps;
	COUT << "\tPrint convergence results at every " << verbose << " steps" << endl;
	return CALL_FFT_PGAUGE(_pgauge, ILambda, DIR, alpha, landautune, stopvalue, maxsteps, verbose, atypeDeltax, useGx, atypeGx);
}	
template complexs
COVGaugeFixingFFT<float>(gauges _pgauge, gauges ILambda, int DIR, float alpha, bool landautune, float stopvalue, int maxsteps, int verbose, ArrayType atypeDeltax, bool useGx, ArrayType atypeGx);
template complexd
COVGaugeFixingFFT<double>(gauged _pgauge, gauged ILambda, int DIR, double alpha, bool landautune, double stopvalue, int maxsteps, int verbose, ArrayType atypeDeltax, bool useGx, ArrayType atypeGx);
